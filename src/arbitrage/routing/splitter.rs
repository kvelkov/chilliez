...
impl RouteSplitter {
    ...
    pub async fn generate_splits(
        ...
        if !self.should_split_route(request, available_paths) {
            debug!("Route splitting not beneficial for trade size {}", (request.amount as f64));
            return Ok(Vec::new());
        }

        info!("Generating route splits for {} {} -> {}", 
              (request.amount as f64), request.input_token, request.output_token);
        ...
        let splits = match strategy {
            SplitStrategy::EqualWeight => {
                self.generate_equal_weight_splits(available_paths, request).await?
            }
            SplitStrategy::LiquidityWeighted => {
                self.generate_liquidity_weighted_splits(available_paths, request).await?
            }
            SplitStrategy::ImpactOptimized => {
                self.generate_impact_optimized_splits(available_paths, request).await?
            }
            SplitStrategy::GasOptimized => {
                self.generate_gas_optimized_splits(available_paths, request).await?
            }
            SplitStrategy::Dynamic => {
                self.generate_dynamic_splits(available_paths, request).await?
            }
    ...
    pub async fn create_optimal_allocation(
        ...
        let total_expected_output: f64 = split_routes.iter()
            .map(|split| split.expected_amount_out)
            .sum();

        let total_gas_cost: u64 = split_routes.iter()
            .map(|split| split.gas_estimate)
            .sum();

        // Calculate execution time considering parallel execution
        let execution_groups = self.create_execution_groups(&split_routes);
        let total_execution_time = self.calculate_total_execution_time(&execution_groups, &split_routes);

        // Calculate improvement over single best route
        let best_single_route = split_routes.iter()
            .max_by(|a, b| a.expected_amount_out.partial_cmp(&b.expected_amount_out).unwrap())
            .unwrap();
        let improvement = ((total_expected_output - best_single_route.expected_amount_out) 
                          / best_single_route.expected_amount_out) * 100.0;
    ...
    /// Determine optimal splitting strategy
    fn determine_optimal_strategy(&self, request: &RouteRequest, paths: &[RoutePath]) -> SplitStrategy {
        // Use dynamic strategy for high-priority trades
        if matches!(request.speed_priority, RoutingPriority::SpeedOptimized | RoutingPriority::Balanced) {
            return SplitStrategy::Dynamic;
        }

        // Choose strategy based on trade characteristics
        let total_liquidity: f64 = paths.iter()
            .map(|path| path.liquidity_score)
            .sum();

        let avg_gas_cost: f64 = paths.iter()
    ...
    async fn generate_equal_weight_splits(
        ...
        for (i, path) in paths.iter().take(num_splits).enumerate() {
            let split = SplitRoute {
                ...
                expected_amount_out: path.expected_output * (amount_per_split / (request.amount as f64)),
    ...
    async fn generate_impact_optimized_splits(
        ...
        sorted_paths.sort_by(|a, b| {
            ...
            impact_a.partial_cmp(&impact_b).unwrap_or(std::cmp::Ordering::Equal)
        });

        for (i, path) in sorted_paths.iter().enumerate() {
            if splits.len() >= self.config.max_splits || remaining_amount < self.config.min_split_amount {
                break;
            }
            ...
            if optimal_amount >= self.config.min_split_amount {
                ...
                let split = SplitRoute {
                    ...
                    expected_amount_out: path.expected_output * (optimal_amount / (request.amount as f64)),
                    execution_priority: i as u8,
                    can_execute_parallel: true,
                    dependency_routes: Vec::new(),
                    gas_estimate: path.estimated_gas_cost,
    ...
    async fn generate_gas_optimized_splits(
        ...
        let mut paths_with_efficiency: Vec<(usize, &RoutePath, f64)> = paths.iter()
            .enumerate()
            .map(|(i, path)| {
        ...
        paths_with_efficiency.sort_by(|a, b| b.2.partial_cmp(&a.2).unwrap_or(std::cmp::Ordering::Equal));
        ...
        for (i, (_, path, _)) in paths_with_efficiency.iter().enumerate() {
            ...
            if amount_in >= self.config.min_split_amount {
                ...
                let split = SplitRoute {
                    ...
                    allocation_percentage,
                    amount_in,
                    expected_amount_out: path.expected_output * (amount_in / (request.amount as f64)),
                    execution_priority: i as u8,
                    can_execute_parallel: true,
                    dependency_routes: Vec::new(),
                    gas_estimate: path.estimated_gas_cost,
    ...
    async fn generate_dynamic_splits(
        ...
        let mut path_scores: Vec<(usize, &RoutePath, f64)> = paths.iter()
            .enumerate()
            .map(|(i, path)| {
    ...
    fn calculate_composite_score(&self, path: &RoutePath, request: &RouteRequest) -> f64 {
        ...
        let speed_score = 1.0 / (path.execution_time_estimate.unwrap_or(Duration::from_millis(500)).as_millis() as f64 / 1000.0);
    ...
    fn can_execute_parallel(&self, path: &RoutePath, all_paths: &[RoutePath]) -> bool {
        ...
        let path_pools: std::collections::HashSet<_> = path.steps.iter()
        ...
        let has_overlap = all_paths.iter().any(|other_path| {
            ...
            other_path.steps.iter().any(|step| path_pools.contains(&step.pool_address))
        });
    ...
    async fn optimize_splits(&self, mut splits: Vec<SplitRoute>, request: &RouteRequest) -> Result<Vec<SplitRoute>> {
        if splits.is_empty() {
            return Ok(splits);
        }
    ...
    fn has_dependency_conflicts(&self, split: &SplitRoute, all_splits: &[SplitRoute]) -> bool {
        let split_pools: std::collections::HashSet<_> = split.route_path.steps.iter()
            .map(|step| step.pool_address)
            .collect();
        ...
        all_splits.iter().any(|other| {
            ...
            other.route_path.steps.iter().any(|step| split_pools.contains(&step.pool_address))
        })
    }
    ...
    fn routes_conflict(&self, route_a: &RoutePath, route_b: &RoutePath) -> bool {
        let pools_a: std::collections::HashSet<_> = route_a.steps.iter()
            .map(|step| step.pool_address)
            .collect();

        route_b.steps.iter().any(|step| pools_a.contains(&step.pool_address))
    }
    ...
    fn calculate_total_execution_time(
        ...
        for group in execution_groups {
            let group_time = group.iter()
                .filter_map(|split_id| {
                    splits.iter().find(|s| &s.id == split_id)
                })
    ...
    fn calculate_split_confidence(&self, splits: &[SplitRoute]) -> f64 {
        ...
        let avg_confidence = splits.iter()
    ...
    fn update_strategy_stats(&mut self, strategy: &SplitStrategy, splits: &[SplitRoute], request: &RouteRequest) {
        let strategy_name = format!("{:?}", strategy);
        let stats = self.stats.strategy_performance.entry(strategy_name).or_insert(StrategyStats {
            usage_count: 0,
            success_rate: 0.0,
            average_improvement: 0.0,
        ...
        stats.average_improvement = (stats.average_improvement * (count - 1.0) + improvement) / count;
        ...
        stats.average_gas_efficiency = (stats.average_gas_efficiency * (count - 1.0) + gas_efficiency) / count;
    }
    ...
    pub async fn split_route(
        ...
        let request = RouteRequest {
            input_token: route.steps.first()
                .map(|s| s.from_token.to_string())
                .unwrap_or_default(),
            output_token: route.steps.last()
        ...
        self.create_optimal_allocation(splits, *amount as f64).await
            .map(Some)
    }
}
...
mod tests {
    ...
    async fn test_equal_weight_splits() {
        ...
        let request = create_test_request();
        let splits = splitter.generate_equal_weight_splits(&paths, &request).await.unwrap();
    ...
    async fn test_liquidity_weighted_splits() {
        ...
        let splits = splitter.generate_liquidity_weighted_splits(&paths, &request).await.unwrap();
    ...
    async fn test_optimal_allocation() {
        ...
        let allocation = splitter.create_optimal_allocation(splits, 10000.0).await.unwrap();
