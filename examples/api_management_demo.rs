...
use solana_arb_bot::{
    config::Config, AdvancedRateLimiter, ApiManager, ApiRequest, RequestPriority, RpcConnectionPool,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    let permit = helius_limiter
        .acquire_permit(critical_request.priority, &critical_request.endpoint)
        .await?;
    ...
    let permit = helius_limiter
        .acquire_permit(trading_request.priority, &trading_request.endpoint)
        .await?;
    ...
    info!(
        "Queuing background request: {:?}",
        background_request.priority
    );

    let permit = helius_limiter
        .acquire_permit(background_request.priority, &background_request.endpoint)
        .await?;
    ...
    for i in 0..20 {
        ...
        let handle = tokio::spawn(async move {
            ...
            match limiter
                .acquire_permit(RequestPriority::Medium, &endpoint)
                .await
            {
                Ok(permit) => {
                    // Simulate some work
    ...
    if let Err(e) = helius_limiter
        .acquire_permit(RequestPriority::Medium, "/test")
        .await
    {
...
async fn demonstrate_request_patterns(limiter: Arc<AdvancedRateLimiter>) -> anyhow::Result<()> {
    ...
    for i in 0..5 {
        ...
        let permit = limiter
            .acquire_permit(RequestPriority::High, &endpoint)
            .await?;
    ...
    for i in 0..3 {
        ...
        let permit = limiter
            .acquire_permit(RequestPriority::Background, &endpoint)
            .await?;
