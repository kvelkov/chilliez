...
async fn test_jupiter_quote_with_fallback() {
    ...
    let _result = client.get_quote_with_fallback(sol_mint, usdc_mint, amount, slippage_bps).await;
...
async fn test_jupiter_quote_error_handling() {
    ...
    let result = client.get_quote_with_fallback("INVALID", "INVALID", 1, 100).await;
...
async fn test_jupiter_swap_transaction_creation() {
    ...
    let quote_result = client.get_quote_with_fallback(sol_mint, usdc_mint, amount, slippage_bps).await;
    ...
    if let Ok(quote) = quote_result {
        ...
        let tx_result = client.create_swap_transaction(&quote, user_pubkey_str, priority_fee).await;
        ...
        println!("Skipping transaction test due to quote failure: {:?}", quote_result);
    }
}
...
async fn test_jupiter_client_rate_limiting() {
    ...
    for i in 0..3 {
        let _result = client.get_quote_with_fallback(sol_mint, usdc_mint, 1_000_000, 50).await;
