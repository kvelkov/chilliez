...
use solana_arb_bot::{
    dex::{
        ...
        clients::phoenix::{OrderBook, OrderSide, OrderType, PhoenixClient, PHOENIX_PROGRAM_ID},
...
fn test_phoenix_market_order_execution() {
    ...
    println!(
        "Debug: filled={}, cost={}, avg_price={}",
        filled, cost, avg_price
    );
    ...
    assert!(
        avg_price >= 200_500_000,
        "Average price should be at least the first level price"
    );

    println!(
        "✅ Market order execution: filled={}, cost=${:.2}, avg_price=${:.2}",
        filled as f64 / 1_000_000.0,
        cost as f64 / 1_000_000.0,
        avg_price as f64 / 1_000_000.0
    );
}
...
fn test_phoenix_price_impact_calculation() {
    ...
    println!(
        "Debug: small_impact={}, large_impact={}",
        small_impact, large_impact
    );

    assert!(
        small_impact >= Decimal::ZERO,
        "Small order impact should be non-negative"
    );
    assert!(
        large_impact >= Decimal::ZERO,
        "Large order impact should be non-negative"
    );
    ...
    assert!(
        small_impact < Decimal::from_str("0.5").unwrap(),
        "Small order should have < 50% impact"
    );
    assert!(
        large_impact < Decimal::ONE,
        "Large order should have < 100% impact"
    );

    println!(
        "✅ Price impact: small_order={:.3}%, large_order={:.3}%",
...
fn test_phoenix_market_metrics() {
    ...
    assert_eq!(
        metrics.mid_price, 200_000_000,
        "Mid price should be $200.00"
    );
    ...
    assert!(
        metrics.bid_depth_1pct > 0,
        "Should have bid depth within 1%"
    );
    assert!(
        metrics.ask_depth_1pct > 0,
        "Should have ask depth within 1%"
    );

    println!(
        "✅ Market metrics: mid=${:.2}, spread={}bps, bid_depth={}, ask_depth={}",
        metrics.mid_price as f64 / 1_000_000.0,
        metrics.spread_bps,
...
fn test_phoenix_optimal_order_size() {
    ...
    let optimal_size_1pct = phoenix::calculate_optimal_order_size(
        &asks, mid_price, 100, // 1%
    )
    .unwrap();

    // Test with 0.5% max price impact (should be smaller or equal)
    let optimal_size_05pct = phoenix::calculate_optimal_order_size(
        &asks, mid_price, 50, // 0.5%
    )
    .unwrap();

    println!(
        "Debug: optimal_size_1pct={}, optimal_size_05pct={}",
        optimal_size_1pct, optimal_size_05pct
    );

    assert!(
        optimal_size_05pct <= optimal_size_1pct,
        "Stricter impact limit should give smaller or equal size"
    );
    // Optimal size should exist (u64 is always >= 0)

    let total_liquidity: u64 = asks.iter().map(|l| l.size).sum();
    assert!(
        optimal_size_1pct <= total_liquidity,
        "Optimal size shouldn't exceed total liquidity"
    );

    println!(
        "✅ Optimal order sizes: 1%_impact={:.2} SOL, 0.5%_impact={:.2} SOL",
        optimal_size_1pct as f64 / 1_000_000.0,
        optimal_size_05pct as f64 / 1_000_000.0
    );
}
...
fn test_phoenix_client_basic() {
    ...
    assert!(
        quote.slippage_estimate.is_some(),
        "Should provide slippage estimate"
    );

    println!(
        "✅ Phoenix client quote: {} -> {} ({}), slippage: {:.2}%",
        quote.input_token,
        quote.output_token,
        quote.output_amount,
...
fn test_phoenix_swap_instruction() {
    ...
    let instruction_result =
        rt.block_on(client.get_swap_instruction_enhanced(&swap_info, pool_info));

    assert!(
        instruction_result.is_ok(),
        "Should build valid swap instruction"
    );
    ...
    assert!(
        ...
        "Should have minimum required accounts"
    );
    assert!(!instruction.data.is_empty(), "Should have instruction data");

    println!(
        "✅ Phoenix swap instruction: {} accounts, {} bytes data",
...
async fn test_phoenix_health_check() {
    ...
    assert!(
        health.response_time_ms.is_some(),
        "Should measure response time"
    );
    assert!(
        health.last_successful_request.is_some(),
        "Should track last request"
    );
    assert_eq!(health.error_count, 0, "Should have no errors initially");

    println!(
        "✅ Phoenix health: healthy={}, response_time={}ms, status='{}'",
        health.is_healthy,
        health.response_time_ms.unwrap(),
...
fn test_order_book_helpers() {
    ...
    assert!(
        best_bid.unwrap() < best_ask.unwrap(),
    ...
    assert!(
        mid > best_bid.unwrap(),
        "Mid price should be above best bid"
    );
    assert!(
        mid < best_ask.unwrap(),
        "Mid price should be below best ask"
    );
    ...
    println!(
        "✅ OrderBook helpers: best_bid=${:.2}, best_ask=${:.2}, mid=${:.2}, impact={:.3}%",
...
fn test_phoenix_error_handling() {
    ...
    assert_eq!(
        impact.unwrap(),
        Decimal::ZERO,
        "Should return zero impact for zero mid price"
    );
    ...
    assert_eq!(
        optimal_size.unwrap(),
        0,
        "Should return zero for empty order book"
    );
...
async fn test_phoenix_full_integration_flow() {
    ...
    let quote = client
        .calculate_onchain_quote(&pool_info, 1_000_000)
        .unwrap();
    ...
    let instruction = client
        .get_swap_instruction_enhanced(&swap_info, Arc::new(pool_info))
        .await
        .unwrap();
    assert_eq!(
        instruction.program_id, PHOENIX_PROGRAM_ID,
        "Should use Phoenix program"
    );
    ...
    println!(
        "✅ Phoenix full integration: quote={} -> {}, instruction built, health OK",
...
fn test_phoenix_math_performance() {
    ...
    assert!(
        ops_per_sec > 1000.0,
        "Should handle > 1000 operations per second"
    );

    println!(
        "✅ Phoenix math performance: {:.0} ops/sec, {:.2}ms total for 3000 operations",
        ops_per_sec,
        duration.as_millis()
    );
}
