...
fn load_config_from_env() -> Result<PaperTradingConfig> {
    ...
    // Parse token mints
    let sol_mint: Pubkey = SOL_MINT.parse().map_err(|e| ArbError::InvalidInput(format!("Invalid SOL mint: {}", e)))?;
    let usdc_mint: Pubkey = USDC_MINT.parse().map_err(|e| ArbError::InvalidInput(format!("Invalid USDC mint: {}", e)))?;
    let usdt_mint: Pubkey = USDT_MINT.parse().map_err(|e| ArbError::InvalidInput(format!("Invalid USDT mint: {}", e)))?;
    ...
    let config = PaperTradingConfig {
        ...
        simulated_tx_fee: env::var("SIMULATED_TX_FEE").unwrap_or_else(|_| "5000".to_string()).parse().unwrap_or(5000),
        simulation_slippage_bps: env::var("MAX_SLIPPAGE_BPS").unwrap_or_else(|_| "50".to_string()).parse().unwrap_or(50),
    ...
    info!("  Initial SOL: {} lamports ({:.3} SOL)", initial_sol, initial_sol as f64 / 1e9);
    info!("  Simulated TX Fee: {} lamports", config.simulated_tx_fee);
    info!("  Max Slippage: {} bps ({:.2}%)", config.simulation_slippage_bps, config.simulation_slippage_bps as f64 / 100.0);
...
async fn test_network_connectivity() -> Result<()> {
    ...
    match rpc_client.get_health().await {
        ...
        Err(e) => {
            warn!("⚠️ Solana RPC connection failed: {} (continuing with simulation)", e);
        }
    }
    
    // Test Jupiter API
    let jupiter_url = env::var("JUPITER_API_URL").unwrap_or_else(|_| "https://quote-api.jup.ag/v6".to_string());
    match reqwest::get(&format!("{}/tokens", jupiter_url)).await {
        Ok(response) if response.status().is_success() => {
            info!("✅ Jupiter API connected: {}", jupiter_url);
        }
        Ok(response) => {
            warn!("⚠️ Jupiter API responded with status: {} (continuing with simulation)", response.status());
        }
        Err(e) => {
            warn!("⚠️ Jupiter API connection failed: {} (continuing with simulation)", e);
        }
    }
...
async fn display_initial_portfolio(portfolio: &SafeVirtualPortfolio) {
    ...
    info!("  SOL:  {} lamports ({:.3} SOL)", sol_balance, sol_balance as f64 / 1e9);
    info!("  USDC: {} micro-units ({:.2} USDC)", usdc_balance, usdc_balance as f64 / 1e6);
    info!("  USDT: {} micro-units ({:.2} USDT)", usdt_balance, usdt_balance as f64 / 1e6);
...
async fn run_real_market_simulation(
    ...
    for (i, scenario) in scenarios.iter().enumerate() {
        info!("🔄 Executing scenario {} of {}: {}", i + 1, scenarios.len(), scenario.id);
        info!("  Route: {} -> {}", scenario.input_token, scenario.output_token);
...
async fn generate_final_report(
    ...
    info!("📊 Opportunities Analyzed: {}", analytics_guard.opportunities_analyzed);
    info!("📈 Opportunities Executed: {}", analytics_guard.opportunities_executed);
    info!("✅ Successful Executions: {}", analytics_guard.successful_executions);
    info!("📊 Execution Rate: {:.2}%", analytics_guard.execution_rate * 100.0);
    info!("📈 Success Rate: {:.2}%", analytics_guard.success_rate * 100.0);
    info!("⚡ Avg Execution Time: {:.2}ms", analytics_guard.avg_execution_time_ms);
    info!("📊 Avg Slippage: {:.2}%", analytics_guard.avg_slippage_bps / 100.0);
    info!("💰 Total Fees Paid: {} lamports", analytics_guard.total_fees_paid);
    info!("📊 Portfolio Summary:");
    info!("  Total Trades: {}", portfolio_summary.total_trades);
    info!("  Successful Trades: {}", portfolio_summary.successful_trades);
    info!("  Success Rate: {:.2}%", portfolio_summary.success_rate * 100.0);
