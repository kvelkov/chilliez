...
impl Quote {
    pub fn profit(&self) -> i64 { self.output_amount as i64 - self.input_amount as i64 }
    pub fn profit_pct(&self) -> f64 {
        if self.input_amount == 0 { 0.0 } 
        else { (self.output_amount as f64 - self.input_amount as f64) / self.input_amount as f64 * 100.0 }
    }
    pub fn output_as_float(&self, decimals: u8) -> f64 { self.output_amount as f64 / 10f64.powi(decimals as i32) }
    pub fn input_as_float(&self, decimals: u8) -> f64 { self.input_amount as f64 / 10f64.powi(decimals as i32) }
}

// ---- Helper methods for PoolInfo ----
impl PoolInfo {
    // These helpers are useful for CLMM pools
    pub fn get_sqrt_price(&self) -> u128 { self.sqrt_price.unwrap_or(0) }
    pub fn get_liquidity(&self) -> u128 { self.liquidity.unwrap_or(0) }
    pub fn get_tick_current_index(&self) -> i32 { self.tick_current_index.unwrap_or(0) }
    pub fn get_tick_spacing(&self) -> u16 { self.tick_spacing.unwrap_or(1) }
    pub fn get_fee_rate_bips(&self) -> u16 { self.fee_rate_bips.unwrap_or(30) }
}