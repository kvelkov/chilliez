...
impl BalanceMonitor {
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        info!(
            "✅ Balance monitor started with {} monitoring tasks",
            monitoring_tasks
        );
        Ok(())
    }
    ...
    pub async fn update_optimistic_balance(&self, account: Pubkey, new_balance: u64) {
        ...
        debug!(
            "🔮 Updated optimistic balance for {}: {} lamports",
            account, new_balance
        );
    }
    ...
    pub async fn check_sufficient_balance(
        ...
        let current_balance =
            if let Some((optimistic, _)) = self.get_optimistic_balance(account).await {
                optimistic
            } else if let Some((confirmed, _)) = self.get_confirmed_balance(account).await {
                confirmed
            } else {
                return Err(anyhow!(
                    "No balance data available for account: {}",
                    account
                ));
            };
    ...
    async fn check_safety_mode_trigger(&self, account: Pubkey, new_balance: u64) -> bool {
        ...
        if let Some(&initial_balance) = initial_balances.get(&account) {
            if initial_balance > 0 {
                let percentage_change = ((new_balance as f64 - initial_balance as f64)
                    / initial_balance as f64)
                    * 100.0;

                if percentage_change <= -self.config.safety_mode_threshold_pct {
                    warn!(
                        "🚨 Safety mode trigger: Account {} balance dropped {:.2}% (from {} to {})",
                        account,
    ...
    async fn handle_account_notification(&self, notification: AccountNotification) -> Result<()> {
        ...
        if let Some(account) = account {
            ...
            info!(
                "💰 Balance update: {} = {} lamports (change: {:+})",
        ...
        // Task 2: Connection health checker
        {
            ...
            tokio::spawn(async move {
                let mut health_check_interval =
                    interval(Duration::from_millis(config.balance_check_interval_ms));
                ...
                loop {
                    ...
                    if heartbeat_age > Duration::from_millis(config.max_balance_age_ms) {
                        warn!(
                            "⚠️ Balance monitor heartbeat is stale ({}ms old)",
    ...
    async fn process_account_value(&self, account_value: &AccountValue) -> Result<()> {
        ...
        debug!(
            "Processing account: lamports={}, executable={}, owner={}",
    ...
    pub async fn process_demo_message(&mut self, message_text: &str) -> Result<()> {
        ...
        if let Ok(notification) = serde_json::from_str::<AccountNotification>(message_text) {
            ...
            debug!(
                "Processing notification: method={}, jsonrpc={}",
    ...
    pub async fn trigger_balance_update_from_event(
        ...
        debug!(
            "🔔 External event triggered balance update for {}: {} ({})",
            account,
    ...
    pub async fn trigger_batch_balance_updates(
        ...
        debug!(
            "🔔 Batch external event triggered {} balance updates ({})",
            updates.len(),
            event_source
        );
        ...
        for (account, change) in updates {
            if let Err(e) = self
                .trigger_balance_update_from_event(account, change, event_source)
                .await
            {
...
mod tests {
    ...
    async fn test_safety_mode_trigger() {
        ...
        let should_trigger = monitor
            .check_safety_mode_trigger(account, new_balance)
            .await;
