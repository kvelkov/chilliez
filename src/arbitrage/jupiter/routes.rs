...
impl JupiterRouteOptimizer {
    ...
    pub fn new(
        ...
        info!("   - Route evaluation timeout: {}ms", config.route_evaluation_timeout_ms);
    ...
    pub async fn find_optimal_route(
        ...
        if !self.config.enabled {
            ...
            return self.single_route_fallback(input_mint, output_mint, amount, slippage_bps).await;
        }

        let start_time = Instant::now();
        info!("🛣️  Finding optimal route: {} -> {} (amount: {})", 
              input_mint.chars().take(6).collect::<String>(),
              output_mint.chars().take(6).collect::<String>(),
              amount);

        // Check route cache first
        if let Some(cached_routes) = self.get_cached_routes(input_mint, output_mint, amount).await {
            debug!("📦 Using cached routes for evaluation");
            return self.evaluate_cached_routes(cached_routes, slippage_bps, start_time).await;
        }

        // Discover multiple routes
        let discovered_routes = self.discover_routes(input_mint, output_mint, amount, slippage_bps).await?;
        ...
        if self.config.caching.enabled {
            self.cache_routes(input_mint, output_mint, amount, &discovered_routes).await;
        }
    ...
    async fn discover_routes(
        ...
        for request in quote_requests.into_iter().take(self.config.max_alternative_routes as usize) {
            ...
            let task = tokio::spawn(async move {
                ...
                client.get_quote_with_fallback(
                    &request.input_mint,
                    &request.output_mint,
                    request.amount,
    ...
    fn deduplicate_routes(&self, routes: Vec<QuoteResponse>) -> Vec<QuoteResponse> {
        ...
        for route in routes {
            ...
            let route_signature = route.route_plan
                .iter()
        ...
        debug!("🔄 Deduplicated {} routes to {} unique patterns", 
               original_count, unique_routes.len());
    ...
    async fn evaluate_routes_parallel(&self, routes: Vec<QuoteResponse>) -> Vec<RouteEvaluation> {
        ...
        debug!("📊 Route evaluation complete: {} successful", evaluations.len());
    ...
    async fn evaluate_single_route(
        ...
        let reliability = {
            ...
            reliability_guard.get(&route_pattern).cloned().unwrap_or_else(|| {
                RouteReliability {
        ...
        let final_score = 
            score_components.output_amount_score * scoring_config.output_amount_weight +
    ...
    fn select_best_route<'a>(&self, evaluations: &'a [RouteEvaluation]) -> Result<&'a RouteEvaluation, ArbError> {
        if evaluations.is_empty() {
            return Err(ArbError::JupiterApiError("No route evaluations available".to_string()));
        }
        ...
        if evaluations.len() > 1 {
            ...
            if improvement_pct < self.config.min_route_improvement_pct {
                debug!("🤏 Route improvement {:.2}% below threshold {:.2}%, using second route", 
                       improvement_pct, self.config.min_route_improvement_pct);
                // Use more reliable route if improvement is marginal
                if second_best.reliability.success_rate > best_by_score.reliability.success_rate + 0.1 {
    ...
    async fn single_route_fallback(
        ...
        let quote = self.jupiter_client.get_quote_with_fallback(
    ...
    async fn get_cached_routes(
        ...
        if let Some(cached_set) = cache.get_mut(&cache_key) {
            ...
            if age < ttl && Self::are_market_conditions_stable(&cached_set.market_conditions) {
                ...
                debug!("📦 Using cached routes (age: {:?}, hits: {})", age, cached_set.hit_count);
    ...
    async fn evaluate_cached_routes(
        ...
        let compatible_routes: Vec<_> = routes.into_iter()
            .filter(|route| {
        ...
        if compatible_routes.is_empty() {
            ...
            return Err(ArbError::JupiterApiError("No compatible cached routes".to_string()));
        }
        ...
        Ok(MultiRouteResult {
            ...
            routes_evaluated,
            routes_failed: 0,
            selection_reason: format!("cached routes: {}", self.generate_selection_reason(best_route)),
        })
    }
    ...
    async fn update_metrics(&self, routes_evaluated: usize, total_time: Duration) {
        if let Ok(_metrics) = self.metrics.try_lock() {
            ...
            debug!("📊 Route optimization metrics: {} routes in {:?}", 
    ...
    fn cleanup_old_cache_entries(
        ...
        let entries: Vec<_> = cache.iter().map(|(k, v)| (k.clone(), v.cached_at)).collect();
...
mod tests {
    ...
    fn create_test_quote(out_amount: u64, price_impact: f64, hop_count: usize) -> QuoteResponse {
        let route_plan = (0..hop_count).map(|i| {
            RoutePlan {
                swap_info: SwapInfo {
    ...
    async fn test_route_scoring() {
        ...
        let evaluation = JupiterRouteOptimizer::evaluate_single_route(
            quote, &scoring_config, reliability_map
        ).await;
    ...
    fn test_amount_bucketing() {
        assert_eq!(JupiterRouteOptimizer::bucket_amount_for_routes(5_000_000), 10_000_000);
        assert_eq!(JupiterRouteOptimizer::bucket_amount_for_routes(15_000_000), 20_000_000);
        assert_eq!(JupiterRouteOptimizer::bucket_amount_for_routes(22_000_000), 20_000_000);
    }
