...
impl FeeManager {
    ...
    pub async fn calculate_multihop_fees(
        ...
        let risk_score = self.calculate_risk_score(total_cost, trade_value_sol * sol_price_usd, &network_congestion);
    ...
    pub fn estimate_compute_units(&self, pools: &[&PoolInfo]) -> u32 {
        ...
        let total_units = base_compute_units + (pools.len() as u32 * per_pool_units) + jupiter_routing_overhead;
    ...
    fn determine_congestion_level(&self, data: &NetworkCongestionData) -> NetworkCongestionLevel {
        let avg_slot_time: f64 = data.recent_slot_times.iter().map(|&x| x as f64).sum::<f64>() 
    ...
    fn calculate_risk_score(&self, total_cost: f64, trade_value_usd: f64, congestion: &NetworkCongestionLevel) -> f64 {
    ...
    pub async fn estimate_fees(
        ...
        Ok(FeeBreakdown {
            ...
            explanation: format!("Base: {}, Priority: {}, DEX: {}", base_fee, priority_fee, dex_fee),
...
impl FeeEstimator {
    ...
    pub async fn estimate_network_fees(&self) -> Result<u64> {
        if let Some(manager) = &self.fee_manager {
            let breakdown = manager.estimate_fees(
