...
use super::rate_limiter::{RateLimiterManager, RequestPriority, RateLimitStats};
use super::connection_pool::{RpcConnectionPool, RpcConnection, EndpointStatus};
...
pub enum ApiError {
    ...
    RequestTimeout { provider: String, endpoint: String, timeout_ms: u64 },
    
    #[error("Max retry attempts ({attempts}) exceeded for {provider}/{endpoint}")]
    MaxRetriesExceeded { provider: String, endpoint: String, attempts: u32 },
...
impl ApiManager {
    ...
    pub async fn execute_request<T, F, Fut>(&self, request: ApiRequest, operation: F) -> Result<ApiResponse<T>, ApiError>
    where
        ...
        Fut: std::future::Future<Output = Result<T>>,
    {
        ...
        while attempt <= self.config.max_retry_attempts {
            match self.try_execute_request(&request, &operation, attempt).await {
                Ok(response) => {
                    self.record_success(&request, start_time.elapsed(), attempt).await;
                    return Ok(response);
                }
                Err(e) => {
                    ...
                    let final_delay = Duration::from_millis(std::cmp::max(0, jittered_delay) as u64);
                    
                    debug!("🔄 Retrying request {}/{} for {}/{} after {:?}", 
                           attempt, self.config.max_retry_attempts, request.provider, request.endpoint, final_delay);
    ...
    async fn try_execute_request<T, F, Fut>(&self, request: &ApiRequest, operation: &F, attempt: u32) -> Result<ApiResponse<T>, ApiError>
    where
        F: Fn(RpcConnection) -> Fut,
        Fut: std::future::Future<Output = Result<T>>,
    {
        ...
        if self.config.enable_rate_limiting {
            ...
            if let Some(limiter) = rate_limiters.get_limiter(&request.provider) {
                let _permit = limiter.acquire_permit(request.priority, &request.endpoint).await
        ...
        let result = if let Some(timeout) = request.timeout {
            tokio::time::timeout(timeout, operation(connection)).await
        ...
        match result {
            Ok(data) => {
                debug!("✅ Request succeeded: {}/{} ({}ms, attempt {})", 
            ...
            Err(e) => {
                warn!("❌ Request failed: {}/{} ({}ms, attempt {}): {}", 
    ...
    async fn start_health_monitoring(&self) {
        ...
        info!("💗 Started health monitoring (interval: {}s)", self.config.health_check_interval_secs);
    }
...
impl std::fmt::Display for ApiStats {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        for (key, metrics) in &self.request_metrics {
            writeln!(f, "  {}: {:.1}% success, {:.1}ms avg, {} total", 
                     key, 
