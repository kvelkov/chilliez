...
use solana_arb_bot::dex::api::{CommonSwapInfo, DexClient};
use solana_arb_bot::dex::clients::orca::OrcaClient;
use solana_arb_bot::dex::math::orca::{calculate_whirlpool_swap_output, validate_pool_state, Q64};
...
async fn test_orca_clmm_quote_calculation() {
    ...
    assert!(
        ...
        "Slippage estimate should be provided"
    );
    ...
    println!(
        "✅ CLMM Quote: {} SOL -> {} USDC (slippage: {:.4}%)",
...
async fn test_orca_clmm_math_accuracy() {
    ...
    assert!(
        swap_result.price_impact >= 0.0,
        "Price impact should be non-negative"
    );
    assert!(
        swap_result.price_impact < 0.1,
        "Price impact should be reasonable (<10%)"
    );
    ...
    println!(
        "✅ CLMM Math: {} SOL -> {} USDC (fee: {} SOL, impact: {:.4}%)",
        input_amount as f64 / 1e9,
...
async fn test_orca_swap_instruction_building() {
    ...
    assert_eq!(
        ...
        solana_arb_bot::dex::clients::orca::ORCA_WHIRLPOOL_PROGRAM_ID
    );
    assert!(
        instruction.accounts.len() >= 10,
        "Should have all required accounts"
    );
    assert!(
    ...
    println!(
        "✅ Swap instruction built successfully with {} accounts and {} bytes of data",
...
async fn test_orca_different_swap_amounts() {
    ...
    for amount in test_amounts {
        ...
        assert!(
            ...
            "Output should be positive for amount {}",
            amount
        );
        assert!(
            quote.output_amount > previous_output,
        ...
        assert!(
            ...
            "Larger swaps should have equal or higher price impact"
        );

        println!(
            "Amount: {} SOL -> {} USDC (impact: {:.4}%)",
            amount as f64 / 1e9,
            quote.output_amount as f64 / 1e6,
            impact * 100.0
        );
...
async fn test_orca_reverse_swap_direction() {
    ...
    assert!(
        quote.output_amount > 0,
        "Reverse swap should produce output"
    );
