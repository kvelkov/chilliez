...
use solana_arb_bot::{
    performance::{
        PerformanceManager, PerformanceConfig, RouteInfo, RouteData, PoolState, StressTestConfig,
    },
    arbitrage::routing::{
        smart_router::{SmartRouterConfig, RouteRequest, RoutingPriority, RouteConstraints},
    },
};
...
async fn demo_performance_initialization() -> Result<()> {
    ...
    info!("   - Max Concurrent Workers: {}", performance_config.max_concurrent_workers);
    info!("   - Pool Cache TTL: {:?}", performance_config.pool_cache_ttl);
    info!("   - Route Cache TTL: {:?}", performance_config.route_cache_ttl);
    info!("   - Quote Cache TTL: {:?}", performance_config.quote_cache_ttl);
...
async fn demo_parallel_processing() -> Result<()> {
    ...
    let quote_tasks: Vec<_> = (0..10).map(|i| {
        move || async move {
            ...
            Ok(format!("DEX-{}: Price={:.2}, Liquidity={}", i, price, liquidity))
        }
    }).collect();
    ...
    info!("   - Successful Tasks: {}", results.iter().filter(|r| r.is_ok()).count());
...
async fn demo_advanced_caching() -> Result<()> {
    ...
    for i in 0..5 {
        ...
        cache_manager.set_pool_state(pool_key.clone(), pool_state.clone()).await;
        info!("   - Cached: {} -> Price: {:.2}", pool_key, pool_state.price);
    }
    ...
    for i in 0..3 {
        ...
        if let Some(cached_data) = cache_manager.get_pool_state(&pool_key).await {
            info!("   - Cache HIT: {} -> Price: {:.2}", pool_key, cached_data.price);
        } else {
    ...
    cache_manager.set_route("SOL_USDC_1000".to_string(), route_info.clone()).await;
    
    if let Some(cached_routes) = cache_manager.get_route("SOL_USDC_1000").await {
        info!("   - Cached Route: {} -> {} (output: {})", 
    ...
    info!("   - Pool Cache Hit Rate: {:.1}%", cache_stats.pool_hit_rate * 100.0);
    info!("   - Route Cache Hit Rate: {:.1}%", cache_stats.route_hit_rate * 100.0);
    info!("   - Quote Cache Hit Rate: {:.1}%", cache_stats.quote_hit_rate * 100.0);
...
async fn demo_smart_router_performance() -> Result<()> {
    ...
    info!("   - Intelligent Routing: {}", router_config.enable_intelligent_routing);
    info!("   - Cross-DEX Routing: {}", router_config.enable_cross_dex_routing);
    ...
    info!("📋 Route Request:");
    info!("   - Path: {} -> {}", route_request.input_token, route_request.output_token);
...
async fn demo_real_time_monitoring() -> Result<()> {
    ...
    for i in 0..5 {
        ...
        info!("      - Memory Usage: {:.1}MB", metrics_summary.memory_usage_mb);
        info!("      - Network Latency: {:.1}ms", metrics_summary.network_latency_ms);
    }
...
async fn demo_stress_testing() -> Result<()> {
    ...
    info!("   - Operations Completed: {}", benchmark_results.operations_completed);
    info!("   - Operations/Second: {:.2}", benchmark_results.operations_per_second);
    info!("   - Average Latency: {:.2}ms", benchmark_results.average_latency.as_millis());
    info!("   - Memory Used: {:.1}MB", benchmark_results.memory_used_mb);
    info!("   - Success Rate: {:.2}%", benchmark_results.success_rate * 100.0);
...
async fn demo_performance_reporting() -> Result<()> {
    ...
    for i in 0..3 {
        ...
        cache_manager.set_pool_state(format!("pool_{}", i), pool_state).await;
    }
