...
impl JitoHandler {
    ...
    pub fn new(
        ...
        info!("   ⚡ Flash loan protection: {}", mev_config.use_flashloan_protection);
        info!("   🌐 Jito block engine: {}", jito_config.block_engine_url);
        info!("   💰 Default tip: {} lamports", jito_config.default_tip_lamports);
    ...
    pub async fn submit_protected_bundle(
        ...
        info!("📦 Submitting MEV-protected bundle with {} opportunities, tip: {} lamports", 
              opportunities.len(), tip_amount);
        ...
        self.submit_bundle(bundle_transactions, Some(tip_amount)).await
    }
    ...
    pub async fn submit_bundle(
        ...
        for attempt in 1..=self.jito_config.max_retries {
            debug!("📤 Bundle submission attempt {}/{}", attempt, self.jito_config.max_retries);
    ...
    async fn submit_bundle_request(&self, encoded_transactions: &[String]) -> Result<String, ArbError> {
        ...
        debug!("📤 Sending bundle request to: {}", self.jito_config.block_engine_url);
    ...
    async fn get_bundle_status(&self, bundle_id: &str) -> Result<BundleStatus, ArbError> {
        let url = format!("{}/bundle-status/{}", self.jito_config.block_engine_url, bundle_id);
...
mod tests {
    ...
    async fn test_priority_fee_calculation() {
        ...
        let base_fee = 10_000;
        let optimal_fee = handler.calculate_optimal_priority_fee(&opportunity, base_fee).await.unwrap();
    ...
    async fn test_mev_risk_analysis() {
        ...
        let risk_score = handler.analyze_mev_risk(&high_profit_opportunity).await.unwrap();
