...
impl LiveUpdateMetrics {
    pub fn log_summary(&self) {
        ...
        info!(
            ...
            applied,
            if received > 0 {
                (applied as f64 / received as f64) * 100.0
            } else {
                0.0
            }
        );
        info!(
            ...
            rejected,
            if received > 0 {
                (rejected as f64 / received as f64) * 100.0
            } else {
                0.0
            }
        );
...
impl LiveUpdateManager {
    ...
    pub fn new(
        ...
        let webhook_service = if app_config.enable_webhooks {
            Some(Arc::new(Mutex::new(WebhookIntegrationService::new(
                app_config.clone(),
            ))))
        } else {
            None
    ...
    async fn process_single_update(
        ...
        debug!(
            "🔄 Applied update for pool {} (latency: {}ms)",
            update.pool_address, latency
        );
    }
    ...
    pub async fn send_update(&self, update: LiveUpdateEvent) -> Result<()> {
        ...
        if now - update.timestamp > self.config.max_update_age_ms {
            warn!(
                "Rejecting stale update for pool {} (age: {}ms)",
                update.pool_address,
                now - update.timestamp
            );
            self.metrics
                .total_updates_rejected
                .fetch_add(1, Ordering::Relaxed);
            return Ok(());
        }
    ...
    pub async fn health_check(&self) -> Result<()> {
        ...
        if last_update > 0 && now - last_update > 30000 {
            // 30 seconds
...
impl LiveUpdateManagerBuilder {
    ...
    pub fn build(self) -> Result<LiveUpdateManager> {
        let hot_cache = self
            .hot_cache
            .ok_or_else(|| anyhow!("Hot cache is required"))?;
        let app_config = self
            .app_config
            .ok_or_else(|| anyhow!("App config is required"))?;
