...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let fallback_manager =
        JupiterFallbackManager::new(jupiter_client, jupiter_config, Arc::clone(&metrics));

    println!("✅ Created JupiterFallbackManager with route optimization enabled");
    println!(
        "   📊 Max parallel routes: {}",
        config.jupiter_max_parallel_routes
    );
    println!(
        "   🔀 Max alternative routes: {}",
        config.jupiter_max_alternative_routes
    );
    println!(
        "   ⏱️  Route evaluation timeout: {}ms",
        config.jupiter_route_evaluation_timeout_ms
    );
    println!(
        "   📈 Min route improvement: {}%",
        config.jupiter_min_route_improvement_pct
    );
    ...
    if fallback_manager.is_route_optimization_enabled() {
        ...
        for (input_name, input_mint, output_name) in examples {
            println!(
                "\n📊 Testing route optimization for {} -> {}",
                input_name, output_name
            );

            let amount = if input_name == "USDC" {
                1_000_000
            } else {
                1_000_000_000
            }; // 1 USDC or 1 SOL
            ...
            println!(
                "   🔍 Discovering optimal routes for {} {}",
                amount as f64
                    / if input_name == "USDC" {
                        1_000_000.0
                    } else {
                ...
                .await
            {
                Ok(result) => {
                    ...
                    println!(
                        "      ⏱️  Evaluation time: {:?}",
                        result.total_evaluation_time
                    );
                    println!("      🏆 Selection reason: {}", result.selection_reason);
                    println!(
                        "      💰 Best output: {} units",
                        result.best_route.quote.out_amount
                    );
                }
                Err(e) => {
                    println!(
                        "   ⚠️  Route optimization failed: {} (expected in demo mode)",
                        e
                    );
                }
            }
