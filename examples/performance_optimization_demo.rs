...
async fn main() -> Result<()> {
    ...
    info!("✅ Performance Manager initialized with {} workers", perf_config.max_concurrent_workers);
    ...
    let quote_tasks: Vec<_> = (0..20).map(|i| {
        move || async move {
            ...
            if i % 20 == 0 {
                ...
                Ok(format!("Quote result from DEX {}: 1.0 SOL = {:.2} USDC", 
                          i % 5, 100.0 + (i as f64 * 0.5)))
            }
        }
    }).collect();
    ...
    info!("✅ Parallel simulations completed: {}/{} successful in {:?}", 
          successful_sims, sim_results.len(), sim_duration);
    ...
    cache_manager.set_pool_state("RaydiumPoolABC123".to_string(), pool_state.clone()).await;
    ...
    match cached_pool {
        Some(pool) => {
            info!("✅ Pool state retrieved from cache: {} reserves, {:.4} fee rate", 
    ...
    use solana_arb_bot::performance::cache::{RouteInfo, RouteData};
    ...
    cache_manager.set_route(route_key.clone(), route_info.clone()).await;
    ...
    match cached_route {
        Some(route) => {
            info!("✅ Route retrieved from cache: {} routes, best output: {} USDC", 
                  route.routes.len(), route.total_output as f64 / 1_000_000.0);
        }
        None => {
    ...
    for result in &benchmark_results {
        info!("📊 Benchmark '{}': {:.1} ops/sec, {:.2}ms avg latency, {:.1}% success rate",
              result.test_name,
    ...
    println!("\nCache Performance:");
    println!("- Pool Cache Hit Rate: {:.1}%", cache_stats.pool_hit_rate * 100.0);
    println!("- Route Cache Hit Rate: {:.1}%", cache_stats.route_hit_rate * 100.0);
    println!("- Quote Cache Hit Rate: {:.1}%", cache_stats.quote_hit_rate * 100.0);
    ...
    println!("=====================================");
    println!("✅ Parallel Processing: Up to {} concurrent workers", perf_config.max_concurrent_workers);
    println!("✅ Cache Performance: {:.1}% average hit rate", 
             (cache_stats.pool_hit_rate + cache_stats.route_hit_rate + cache_stats.quote_hit_rate) / 3.0 * 100.0);
    println!("✅ System Throughput: {:.1} operations/second", metrics_summary.throughput_ops_per_sec);
    println!("✅ Average Latency: {:.1}ms", metrics_summary.network_latency_ms);
    println!("✅ System Health: {:.1}%", metrics_summary.system_health_score * 100.0);
