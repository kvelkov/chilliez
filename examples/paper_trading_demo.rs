...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  Input: {} {}",
        opportunity.input_amount, opportunity.input_token
    );
    println!(
        "  Expected Output: {} {}",
        opportunity.expected_output, opportunity.output_token
    );
        ...
        match engine.simulate_arbitrage_execution(&opportunity, &empty_dex_clients).await {
            Ok(result) => {
                ...
                if result.success {
                    ...
                    println!(
                        "    Error: {}",
                        error_message_for_display.unwrap_or_default()
                    );
                }
    ...
    println!(
        "  Average Profit/Trade: {:.0} lamports",
        analytics_guard.get_average_profit_per_trade()
    );
...
fn create_sample_pool_info(token_a: Pubkey, token_b: Pubkey) -> solana_arb_bot::utils::PoolInfo {
    use solana_arb_bot::utils::{DexType, PoolToken};
