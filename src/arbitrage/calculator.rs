...
use dashmap::DashMap;
use once_cell::sync::Lazy; // Ensures that shared caches (and DexType via TokenAmount usage) are initialized once.
use solana_sdk::pubkey::Pubkey;
use log::{debug, warn, info};

// -------------------------------------------------------------------------
// Data Structures and Global Caches
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// Simple Opportunity Calculation (2-hop)
// -------------------------------------------------------------------------
...
pub fn calculate_simple_opportunity_result(
    ...
    // Determine which token in pool_a is being used as input and consequently determine the intermediate token.
    let (pool_a_input_reserve, pool_a_output_reserve, pool_a_input_decimals, pool_a_output_decimals, pool_a_fee_num, pool_a_fee_den, pool_a_intermediate_mint) =
        if pool_a.token_a.mint == *input_token_mint_for_pool_a {
            ...
            return OpportunityCalculationResult { 
                ...
                output_amount: 0.0, 
                profit: -input_amount_tokens, 
                profit_percentage: -1.0, 
                price_impact: 1.0 
            };
        };

    // Convert the float input to its atomic (smallest) unit.
    let input_amount_atomic_pool_a = TokenAmount::from_float(input_amount_tokens, pool_a_input_decimals).amount;

    // Calculate effective input after fees and compute pool_a's intermediate output.
    ...
    // For pool_b, determine which token is the input and which is the final output.
    ...
    // Estimate price impact for both hops for consistency.
    ...
    let price_impact_a = estimate_price_impact(
        ...
        TokenAmount::from_float(input_amount_tokens, pool_a_input_decimals)
    );
...
// -------------------------------------------------------------------------
// Maximum Profit Calculation (2-hop)
// -------------------------------------------------------------------------
...
pub fn calculate_max_profit_result(
    ...
    if p_b_output_token_info.decimals != p_a_input_token_info.decimals {
         warn!("Decimal mismatch for final output. Original input decimals: {}, Pool B output decimals: {}. This may indicate a non-cyclic path or error.", p_a_input_token_info.decimals, p_b_output_token_info.decimals);
    }
...
pub fn estimate_price_impact(
    ...
    if input_reserve_float < 1e-9 && input_amount_after_fee_atomic_float < 1e-9 { 
        return 0.0; 
    }
    ...
    (input_amount_after_fee_atomic_float / denominator).max(0.0).min(1.0)
}

// -------------------------------------------------------------------------
// Multi-hop Profit and Slippage Calculation
// -------------------------------------------------------------------------
...
pub fn calculate_multihop_profit_and_slippage(
    ...
    if pools.is_empty() || directions.is_empty() || pools.len() != directions.len() || (last_fee_data.len() != pools.len() && !last_fee_data.is_empty()) {
        warn!("calculate_multihop_profit_and_slippage called with invalid inputs. Pools: {}, Directions: {}, Fees: {}", pools.len(), directions.len(), last_fee_data.len());
        if pools.is_empty() {
    ...
    let fees_to_use = if last_fee_data.len() != pools.len() {
        ...
        vec![(None, None, None); pools.len()]
    } else {
        last_fee_data.to_vec()
    };
    ...
    for (idx, &pool_ref) in pools.iter().enumerate() {
        ...
        if current_amount_atomic.decimals != input_token_this_hop_decimals {
            warn!("Decimal mismatch for hop {}: current_amount_atomic.decimals ({}) != input_token_this_hop_decimals ({} for token {} in pool {}). Auto-adjusting.", idx, current_amount_atomic.decimals, input_token_this_hop_decimals, input_token_this_hop_mint, pool.name);
    ...
    let fee_breakdown = FeeManager::estimate_multi_hop_with_model(
        pools, &amounts_for_fee_est, directions, &fees_to_use, &slippage_model,
    );
...
// -------------------------------------------------------------------------
// Placeholders for Price and Rebate Calculations
// -------------------------------------------------------------------------
...
fn get_dex_fee_rebate_percentage(dex_type: &crate::utils::DexType) -> f64 {
    match dex_type {
        crate::utils::DexType::Orca => 0.10,
        crate::utils::DexType::Raydium => 0.05,
        crate::utils::DexType::Lifinity => 0.08,
        crate::utils::DexType::Meteora => 0.07,
        crate::utils::DexType::Phoenix => 0.03,
        crate::utils::DexType::Whirlpool => 0.12,
        crate::utils::DexType::Unknown(_) => 0.0,
    }
}
...
pub fn calculate_rebate(
    ...
    if pools.len() != amounts.len() || pools.len() != directions.len() {
        warn!("calculate_rebate: pools, amounts, or directions slices mismatched. Pools: {}, Amounts: {}, Directions: {}.", pools.len(), amounts.len(), directions.len());
        return 0.0;
    }
    for i in 0..pools.len() {
        let pool = pools[i];
        let input_amount_for_hop = &amounts[i];
        ...
        if input_amount_for_hop.decimals != expected_input_decimals {
            warn!("calculate_rebate: Decimal mismatch for input amount in hop {} for pool {}. Expected {}, got {}. Skipping.", i, pool.name, expected_input_decimals, input_amount_for_hop.decimals);
            continue;
        }
        ...
        debug!("Rebate for hop {} (Pool: {}, DEX: {:?}): {:.6} {} (USD {:.4})", i, pool.name, pool.dex_type, rebate_in_hop_input_token, input_token_symbol_for_hop, rebate_for_hop_usd);
    }
