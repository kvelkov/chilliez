...
use solana_arb_bot::{
    dex::{
        api::{DexClient, PoolDiscoverable, CommonSwapInfo},
        clients::meteora::{MeteoraClient, MeteoraPoolType, METEORA_DYNAMIC_AMM_PROGRAM_ID, METEORA_DLMM_PROGRAM_ID},
        math::meteora::{calculate_dynamic_amm_output, calculate_dlmm_output},
    },
...
async fn test_dynamic_amm_swap_instruction() -> Result<()> {
    ...
    let instruction = client.get_swap_instruction_enhanced(&swap_info, pool).await?;
    
    assert_eq!(instruction.program_id, METEORA_DYNAMIC_AMM_PROGRAM_ID);
    assert!(!instruction.accounts.is_empty());
    assert!(!instruction.data.is_empty());
    
    println!("Dynamic AMM instruction created with {} accounts", instruction.accounts.len());
...
async fn test_dlmm_swap_instruction() -> Result<()> {
    ...
    println!("DLMM instruction created with {} accounts", instruction.accounts.len());
...
async fn test_pool_discovery() -> Result<()> {
    ...
    println!("Discovered {} pools: {} Dynamic AMM, {} DLMM", 
        pools.len(),
        pools.iter().filter(|p| client.get_pool_type(p) == MeteoraPoolType::DynamicAmm).count(),
        pools.iter().filter(|p| client.get_pool_type(p) == MeteoraPoolType::Dlmm).count()
    );
...
async fn test_dlmm_math() -> Result<()> {
    ...
    println!("DLMM: {} input -> {} output (bin {}, step {})", 
...
async fn test_quote_edge_cases() -> Result<()> {
    ...
    println!("Edge cases: 0 -> {}, 1 -> {}, 1B -> {}", 
        quote_zero.output_amount,
        quote_small.output_amount,
        quote_large.output_amount
    );
...
async fn test_quote_performance() -> Result<()> {
    ...
    println!("Performance: {} quotes in {:?} (avg: {}µs per quote)", 
