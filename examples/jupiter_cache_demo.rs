...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match jupiter_client.get_quote_with_fallback(sol_mint, usdc_mint, amount, slippage_bps).await {
        Ok(quote) => {
            let duration = start.elapsed();
            println!("   ✅ Quote received: {} SOL -> {} USDC", quote.in_amount, quote.out_amount);
    ...
    match jupiter_client.get_quote_with_fallback(sol_mint, usdc_mint, amount, slippage_bps).await {
        Ok(quote) => {
            let duration = start.elapsed();
            println!("   ✅ Quote received: {} SOL -> {} USDC", quote.in_amount, quote.out_amount);
            println!("   ⏱️  Duration: {:?} (should be much faster from cache)", duration);
        }
        Err(e) => {
            println!("   ❌ Quote failed: {} (this is expected in demo mode)", e);
        }
    ...
    if let Some(stats) = jupiter_client.get_cache_stats().await {
        println!("   📊 Final cache stats: {} hits, {} misses", stats.hits, stats.misses);
    ...
    match jupiter_client.get_quote_with_fallback(sol_mint, usdc_mint, bucketed_amount, slippage_bps).await {
        Ok(quote) => {
            println!("   ✅ Bucketed quote: {} SOL -> {} USDC", quote.in_amount, quote.out_amount);
            println!("   📦 Amount {} bucketed with {}", bucketed_amount, amount);
        }
        Err(e) => {
            println!("   ❌ Bucketed quote failed: {} (this is expected in demo mode)", e);
        }
    }
    
    // Demonstrate cache management
    println!("\n🧹 Cache management demo:");
    println!("   - Cache is enabled: {}", jupiter_client.is_cache_enabled());
