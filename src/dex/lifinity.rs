...
use serde::{Deserialize, Serialize}; // Added Serialize for caching
...
pub struct LifinityApiQuote {
    ...
    pub dex: String, // Lifinity might specify which internal DEX/pool it used
}
...
impl LifinityClient {
    pub fn new(cache: Arc<Cache>, quote_cache_ttl_secs: Option<u64>) -> Self {
        ...
        Self {
            ...
            cache,
            quote_cache_ttl_secs: quote_cache_ttl_secs.unwrap_or(60), // Default 60s TTL
        }
    }
    ...
    fn build_request_with_api_key(&self, url: &str) -> reqwest::RequestBuilder {
        ...
        if !self.api_key.is_empty() {
            req_builder = req_builder.header("api-key", &self.api_key); // Verify header name
        }
        req_builder
    }
...
impl UtilsPoolParser for LifinityPoolParser {
    ...
    fn get_program_id() -> Pubkey { // Changed to return Pubkey directly
        Pubkey::from_str(LIFINITY_PROGRAM_ID)
            ...
            .expect("Static Lifinity program ID should be valid") // Panic if invalid
    }
