...
impl LongTermValidator {
    ...
    pub async fn run_validation(&mut self) -> Result<ValidationResults> {
        ...
        info!(
            "Duration: {:.1} hours",
            self.config.total_duration.as_secs_f64() / 3600.0
        );
    ...
    async fn execute_trading_cycle(&mut self, cycle_id: u64) -> Result<()> {
        ...
        info!(
            "Cycle {}: Simulated: ${:.2}, Actual: ${:.2}, Diff: {:.3}%",
    ...
    async fn get_actual_trade_result(
        &self,
    ...
    fn calculate_difference(&self, simulated: &TradeResult, actual: &TradeResult) -> f64 {
        ...
        let difference =
            ((simulated.actual_profit - actual.actual_profit) / actual.actual_profit).abs() * 100.0;
        difference
    }
    ...
    fn update_validation_stats(&mut self, difference: f64) {
        ...
        self.validation_results.average_difference =
            (self.validation_results.average_difference * (total_comparisons - 1.0) + difference)
                / total_comparisons;

        // Check if validation is still passing
        if difference > self.config.max_allowed_difference_percent {
            warn!(
                "⚠️ Validation threshold exceeded: {:.2}% > {:.2}%",
    ...
    async fn log_status_update(&self, start_time: Instant) {
        ...
        info!(
            ...
            if self.validation_results.total_trades > 0 {
                self.validation_results.successful_trades as f64
                    / self.validation_results.total_trades as f64
                    * 100.0
            } else {
                0.0
            }
        ...
        info!(
            "  Portfolio Value: ${:.2} ({:+.2}%)",
            current_portfolio_value, current_return
        );
        info!(
            "  Max Difference: {:.2}%",
    ...
    async fn complete_validation(&mut self, start_time: Instant) -> Result<()> {
        ...
        let total_expected_trades =
            self.config.total_duration.as_secs() / self.config.trading_interval.as_secs();
        ...
        self.validation_results.validation_passed = self.validation_results.max_difference_observed
...
async fn main() -> Result<()> {
    ...
    println!(
        "- Duration: {:.1} minutes (demo mode)",
        config.total_duration.as_secs_f64() / 60.0
    );
    println!(
        "- Trading Interval: {} seconds",
        config.trading_interval.as_secs()
    );
    println!("- Starting Balance: ${:.2}", config.starting_balance);
    println!(
        "- Max Allowed Difference: {:.1}%",
        config.max_allowed_difference_percent
    );
    println!();
...
mod tests {
    ...
    async fn test_validator_initialization() {
        ...
        assert_eq!(
            validator.validation_results.starting_portfolio_value,
            100_000.0
        );
    }
